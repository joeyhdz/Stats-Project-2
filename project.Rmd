...
<h4>Base Additive Model:</h4>
  ...
  - ANOVA: In the presence of other variables, variables such as priorfrac, age, bmi(right on the cusp of rejection), bonemed, and bonetreat are significant, while weight,height, bonemed_fu, premeno, momfrac, armassist, smoke, fracscore are not significant at the .05 level of significance. 
...
<h4>Base Adj. Model:</h4>
  ...
  - base.adj model removes 6 predictors that were in the base.model and yields a slightly improved AIC of 384.73 compared to base.model AIC 387.59.

  - The p-value from our anova of .1659 > .05 suggests that the difference in deviance between the two models is not statistically significant. This means that we fail to reject the null hypothesis and cannot conclude that Model 1 fits the data significantly better than Model 2, even though it has more predictors. We will proceed with Model 2 (base.adj model)
...
<h4>Base.adj Model Coefficients and CIs:</h4>
```{r, message=F, warning=F, fig.align='center'}
...
<h4>Reducing the model predictors:</h4>
  ...
  - although the base.adj model doesn't perform as well as the red.model, it's features allow for real world applicability in predicting fractures. While it would be nice for most of the explanation to be summed up and accounted for over 4 variables the human body has many factors that can contribute to bone health such as hormones, lifestyle, activity level, etc. It is for these reasons that we will acutally proceed with both models in hand as we look for the appropriate application. 
...
confusionMatrix(pred_label, test$fracture, positive = "Yes")
...
# Interaction Investigation Objective 2
...
# print confusion matrix
(cm <-confusionMatrix(pred_label, test$fracture, positive = "Yes"))
...
(cm <- confusionMatrix(data = knn.preds, reference = as.factor(test$fracture), positive = "Yes"))
...
confusionMatrix(data = class_pred, reference = test$fracture, positive = "Yes")
